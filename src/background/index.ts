...
class Background {
  ...
  public async subscribeTeamNotification(team: Team) {
    ...
    await axios.get(`https://tower.im/teams/${team.guid}`, {
      withCredentials: true
    });

    // 订阅私有频道
    const privateChannel = this.pusher.subscribe(
      `private-member-${team.member.id}`
    );
  ...
  public async start() {
    ...
    // 订阅所有团队频道
    for (const team of Object.keys(this.teams).map(
      (key: string) => this.teams[key]
    )) {
